<?php
    ...
    $polaczenie = @new mysqli($host, $db_user, $db_password, $db_name);
        ...
        if ($rezultat = @$polaczenie->query("SELECT count(post.id) as commentno FROM post join user on post.user_id = user.id WHERE user.id=$id")){
                    
            $ilu_watkow = $rezultat->num_rows;
            if($ilu_watkow>0){  
                $wiersz = $rezultat->fetch_assoc();
                $_SESSION['ile_postow'] = $wiersz['commentno'];
                echo $_SESSION['ile_postow'];
        ...
        <div class="main col-sm-12 col-md-10 offset-sm-0 offset-md-1 border border-light mb-1"
            ...
            <div class="area">
                <div class="subjectrow row-fluid" style="border-radius: 15px 50px 15px 50px;">
                    <p style="padding-left:20px;"><?=$_SESSION['nick']?></p>
                </div>
                <div class="post row">
                                ...
                                <table class="usertable table bg-light">
                                        ...
                                        <tbody>
                                          <tr>
                                            ...
                                            <td class="border border-light"><?=$_SESSION['ile_postow'];?></td>
                                            <td class="border border-light">
                                                <?php
                                                if(isset($_SESSION['ostatnia_aktywnosc'])&&$_SESSION['ostatnia_aktywnosc']!=NULL)
                                                echo $_SESSION['ostatnia_aktywnosc'];
                                                else echo "NULL";
                                                ?>
                                            </td>
                                            <td class="border border-light">
                        ...
                        <div class="row">
                            <div class="usersection container mx-auto border border-dark">
                                ...
                                <div class="row-fluid">
                                    ...
                                    <textarea class="form-control" id="exampleFormControlTextarea347" rows="3" name ="Description"><?=$_SESSION['opis'];?></textarea>
